...
class RobotiqGripperDriver:
    ...
    def _set_vars(self, var_dict: OrderedDict[str, Union[int, float]]):
        ...
        Args:
            var_dict: Dictionary of variables to set (variable_name, value).
    ...
    def _get_var(self, variable: str):
        """Sends the appropriate command to retrieve the value of a variable from the gripper.

        Blocking until the response is received or the socket times out.
    ...
    @staticmethod
    def _is_ack(data: Union[str, bytes]):
        return data == b'ack'
    ...
    def is_active(self):
        ...
        return RobotiqGripperDriver.GripperStatus(status) == RobotiqGripperDriver.GripperStatus.ACTIVE
    ...
    def auto_calibrate(self, log: bool = True) -> None:
        ...
        Args:
            log: Whether to print the results to log.
        """
        # first try to open in case we are holding an object
        (position, status) = self.move_and_wait_for_pos(self.get_open_position(), 64, 1)
        if RobotiqGripperDriver.ObjectStatus(status) != RobotiqGripperDriver.ObjectStatus.AT_DEST:
    ...
    def move(self, position: int, speed: int, force: int) -> Tuple[bool, int]:
        ...
        Args:
            position: Position to move to [min_position, max_position].
            speed: Speed to move at [min_speed, max_speed].
            force: Force to use [min_force, max_force].

        Returns:
            A tuple with a bool indicating whether the action it was successfully sent,
            and an integer with the actual position that was requested, after being
            adjusted to the min/max calibrated range.
        """
    ...
    def move_and_wait_for_pos(self, position: int, speed: int, force: int) -> Tuple[int, ObjectStatus]:  # noqa
        ...
        Returns:
            A tuple with an integer representing the last position returned by the gripper
            after it notified that the move had completed, a status indicating how the move
            ended (see ObjectStatus enum for details). Note that it is possible that the
            position was not reached, if an object was detected during motion.
        """
        ...
        while RobotiqGripperDriver.ObjectStatus(cur_obj) == RobotiqGripperDriver.ObjectStatus.MOVING:
            cur_obj = self._get_var(self.OBJ)

        # report the actual position and the object status
        final_pos = self._get_var(self.POS)
        final_obj = cur_obj
        return final_pos, RobotiqGripperDriver.ObjectStatus(final_obj)